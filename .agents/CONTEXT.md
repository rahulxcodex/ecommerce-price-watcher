...
- [x] Phase 17: Security Hardening, Rate Limiting, Serverless Reliability & Scraper Concurrency (Eliminated hardcoded fallback session secret in `src/lib/auth.ts` by failing on missing secret; implemented email/IP rate limiting and exponential lockout on `/api/auth/signin`; built serverless-persistent Supabase-backed rate limiter in `src/lib/rate-limit.ts` with migration `010_add_rate_limits.sql`; made Supabase mandatory for auth on serverless production eliminating ephemeral `/tmp` writes; unified role and access derivation via `resolveUserRole` to prevent stale token drift; hardened `deriveTitleFromUrl` across 6 platforms with heuristic fallback and error logging; restricted CORS on `/api/products` to trusted origins and blocked unauthorized cross-origin requests; optimized `check-prices.ts` with a 4-worker bounded concurrency pool and domain-aware pacing; added 15s `AbortController` timeout to `fetchWithBackoff`; pooled Playwright Chromium via `withSharedBrowserPage` cutting runner memory/overhead; 100% test suites passing, clean production build).
