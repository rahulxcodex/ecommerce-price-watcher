...
- [x] Phase 16: Multi-Tier Resilient Scraper Architecture & Product-Level Refresh (Engineered 5-tier Amazon scraper with Desktop fast path, Mobile HTTP WAF/Akamai bypass with `Sec-CH-UA-Mobile`, embedded Twister state regex extraction, and Playwright Chromium headless fallback; added round-robin platform queue interleaving and domain-aware adaptive jitter in `check-prices.ts`; added 1-click on-card product refresh and retry button on `ProductCard` and `ProductDetailPage`; enabled single-product recheck in `/api/scraper/trigger` for all authenticated users; verified with clean production build and deployment).
