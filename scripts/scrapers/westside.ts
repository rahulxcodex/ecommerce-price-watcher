...
import { parsePrice, isPlaywrightAvailable, withSharedBrowserPage } from './utils';
...
export async function scrapeWestside(url: string): Promise<ScrapeResult> {
  ...
  if (!isPlaywrightAvailable()) {
    return {
      success: false,
      error:
        'Could not extract Westside product price automatically due to store anti-bot protections. Please enter the current price manually or use the PriceWatcher Companion Extension.',
    };
  }
